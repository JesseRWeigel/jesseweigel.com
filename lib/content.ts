...
/**
 * Optional per-project metrics, generated by scripts/sync-mineflayer-stats.mjs
 * into content/data/<slug>-sessions.json. Returns null when no snapshot exists.
 */
export async function getProjectStats(slug: string): Promise<ProjectStats | null> {
  const filePath = path.join(CONTENT_DIR, 'data', `${slug}-sessions.json`)
  try {
    const raw = await fs.readFile(filePath, 'utf-8')
